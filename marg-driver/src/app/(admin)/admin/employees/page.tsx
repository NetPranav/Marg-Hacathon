...
import { Users, Plus, Mail, User, Phone, CheckCircle2, Copy, Shield, Building2 } from 'lucide-react';
...
export default function EmployeesPage() {
  ...
  const handleProvision = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProvisionError(err.response?.data?.message || 'Failed to provision employee');
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-semibold text-brand-text mb-1 tracking-tight">Organization Employees</h1>
          <p className="text-brand-muted">Manage back-office and warehouse staff</p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-brand-muted/10">
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="py-12 text-center text-brand-muted">Loading employees...</td>
                </tr>
              ) : employees.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <Users className="w-12 h-12 text-brand-muted/30 mb-3" />
                      <p className="text-brand-muted">No employees found. Provision one to get started.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={emp.id} className="hover:bg-brand-bg/30 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-brand-text">{emp.first_name} {emp.last_name}</p>
                          <p className="text-xs text-brand-muted">ID: EMP-{emp.id.toString().padStart(4, '0')}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-sm text-brand-text">{emp.email}</p>
                      <p className="text-xs text-brand-muted">{emp.phone_number}</p>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${emp.is_active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
          ...
          <div className="bg-brand-surface rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {provisioningStatus === 'success' && newCredentials ? (
              <div className="p-8 text-center space-y-6">
                ...
                <div>
                  <h3 className="text-2xl font-semibold text-brand-text mb-2">Employee Provisioned!</h3>
                  <p className="text-brand-muted">Share these temporary credentials with the employee. They will be required to change their password on first login.</p>
                </div>
              ...
              <>
                ...
                <div className="p-6">
                  <form onSubmit={handleProvision} className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-brand-muted text-[13px] font-medium ml-2 uppercase tracking-wider">Department</label>
                      <div className="relative">
                        <Building2 className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-brand-muted" />
                        <select
                          value={provisionData.department}
                          onChange={e => setProvisionData({...provisionData, department: e.target.value})}
                          className="w-full bg-brand-bg/50 border border-brand-muted/20 rounded-[1.2rem] py-3 pl-12 pr-4 focus:outline-none focus:border-brand-orange appearance-none"
                        >
                    ...
                    <div className="pt-4 flex gap-3">
                      ...
                      >
                        {provisioningStatus === 'loading' ? 'Provisioning...' : 'Create Account'}
                      </button>
                    </div>
                  </form>
                </div>
